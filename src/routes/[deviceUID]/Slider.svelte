<script lang="ts">
  ...
  onMount(() => {
    ...
    allRanges.forEach((wrap) => {
      const frequency: HTMLInputElement | null =
        wrap.querySelector('#sampleFrequency');
      const popup: HTMLOutputElement | null =
        wrap.querySelector('.frequencyPopup');
    ...
    sampleFrequencyFull.subscribe((value) => {
      ...
      if (allRanges.length > 0) {
        const frequency: HTMLInputElement | null =
          allRanges[0].querySelector('#sampleFrequency');
        const popup: HTMLOutputElement | null =
          allRanges[0].querySelector('.frequencyPopup');
